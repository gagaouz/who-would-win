...
// ============================================================================
// ⚠️  DEAD CODE — DO NOT EDIT THIS FILE
// ----------------------------------------------------------------------------
// The app's actual entry point is KidsHomeView (see WhoWouldWinApp.swift).
// This `HomeView` has ZERO call sites — it's a legacy pre-Kids-redesign view
// kept only for reference. Editing it has no visible effect; an earlier
// session lost a long debugging arc to that exact confusion.
//
// 👉  If you need to change the home screen, open Views/KidsHomeView.swift.
// 👉  Same goes for the other un-prefixed legacy files:
//     AnimalPickerView.swift, SettingsView.swift, BattleView.swift,
//     PreBattleSheet.swift — replaced by their Kids* counterparts.
// ============================================================================
...
struct HomeView: View {
    ...
    @State private var titleAnim = false   // boolean toggle, exact LET'S BATTLE pattern
    @State private var bounceAnim = false  // boolean toggle, exact LET'S BATTLE pattern
    ...
    var body: some View {
            ...
            ZStack {
                ...
                GeometryReader { geo in
                  ...
                  VStack(spacing: 0) {
                    ...
                    // Two hero animals — boolean-conditional offset, same pattern as LET'S BATTLE
                    HStack(alignment: .center, spacing: 0) {
                        Text(heroPairs[pairIndex].0)
                            .font(.system(size: isIPad ? 140 : 80))
                            .shadow(color: Theme.orange.opacity(0.36), radius: isIPad ? 24 : 12, x: 0, y: 0)
                        ...
                        Text(heroPairs[pairIndex].1)
                            .font(.system(size: isIPad ? 140 : 80))
                            .shadow(color: Theme.cyan.opacity(0.36), radius: isIPad ? 24 : 12, x: 0, y: 0)
                    }
                    .padding(.horizontal, isIPad ? 60 : 40)
                    .offset(y: bounceAnim ? -10 : 0)
                    .padding(.bottom, isIPad ? 40 : 28)

                    // Title — boolean-conditional rotate+scale, same pattern as LET'S BATTLE
                    VStack(spacing: isIPad ? 8 : 4) {
                        VStack(spacing: isIPad ? 2 : -2) {
                            Text("ANIMAL")
                                ...
                                .shadow(color: Theme.orange, radius: 0, x:  2, y: -2)
                                .shadow(color: Theme.orange, radius: 0, x: -2, y:  2)
                                .shadow(color: Theme.yellow.opacity(titleGlow ? 0.8 : 0.3),
                                        radius: titleGlow ? (isIPad ? 28 : 18) : (isIPad ? 14 : 8))
                            Text("VS ANIMAL")
                        ...
                        .scaleEffect(titleAnim ? 1.08 : 1.0)
                        .rotationEffect(.degrees(titleAnim ? 4 : -4))
                    ...
                    if settings.currentStreak >= 2 {
                        HStack(spacing: 8) {
                            ...
                            Text("\(settings.currentStreak) day streak!")
                                .font(Theme.bungee(isIPad ? 17 : 13))
        ...
        .onAppear {
            ...
            withAnimation(.easeInOut(duration: 1.0).repeatForever(autoreverses: true)) { bounceAnim = true }
            withAnimation(.easeInOut(duration: 0.8).repeatForever(autoreverses: true)) { titleAnim  = true }
    ...
    private func packLabel(_ pack: PackInfo) -> some View {
        ...
        VStack(spacing: 2) {
            Text(pack.emoji)
                ...
                .shadow(color: unlocked ? pack.color.opacity(0.48) : .clear, radius: 6, x: 0, y: 0)
            Text(pack.label)
                .font(Theme.bungee(8))
