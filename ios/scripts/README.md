...
Archive provisioning updates are disabled unless `--allow-provisioning-updates` is explicitly provided. For the existing account, add `--credential-config /absolute/path/to/trusted/deploy.sh` to the archive command; this parses only the configured ASC identifiers and passes the existing private-key file to Xcode. Key contents are never read by this helper, and authentication identifiers/path are redacted from its saved command/log. Internal export/upload is a separate root-owned step: preserve the manually allocated build with `manageAppVersionAndBuildNumber=false`, and restrict distribution with `testFlightInternalTestingOnly=true`. Recheck ASC immediately before uploading; the reader does not reserve build numbers. The tools retain symbols and verify version/build, canonical identity, API environment, privacy manifest, signature, Game Center, iCloud, and production App Attest. Any later upload must use the reviewed archive, and completion still requires Apple processing plus owner installation availability.
