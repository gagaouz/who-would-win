...
function clientFingerprint(req: Request): string {
  ...
  return createHmac('sha256', salt).update(`${ip}:${validInstallId}`).digest('hex');
}

function fixedWindowStart(windowMs: number): number {
...
async function increment(profile: LimitProfile, fingerprint: string): Promise<number> {
  ...
  const result = await pool.query<{ count: number }>(
    `INSERT INTO rate_limit_windows (profile, fingerprint, window_start, count, expires_at)
     ...
     RETURNING count`,
    [profile.name, fingerprint, start, profile.windowMs],
  );
  return result.rows[0]?.count ?? profile.max + 1;
}

async function allowedByGlobalLimit(profile: LimitProfile): Promise<boolean> {
  if (!profile.globalMax) return true;
  const count = await increment(
    { ...profile, name: `global:${profile.name}`, max: profile.globalMax },
    'global',
  );
  return count <= profile.globalMax;
}

function limiter(profile: LimitProfile): RequestHandler {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    let concurrencyClaimed = false;
    const release = () => {
      if (!concurrencyClaimed) return;
    ...
    try {
      ...
      release();
      console.error('[rate-limit] shared limiter unavailable:', (error as Error).message);
...
export const battleRateLimit = limiter({
  name: 'battle', max: positiveInt(process.env.BATTLE_LIMIT_PER_HOUR, 30),
  windowMs: 60 * 60 * 1000, globalMax: positiveInt(process.env.BATTLE_GLOBAL_PER_HOUR, 2_000), concurrent: true,
});

export const quickRateLimit = limiter({
  name: 'quick', max: positiveInt(process.env.QUICK_LIMIT_PER_HOUR, 90),
  windowMs: 60 * 60 * 1000, globalMax: positiveInt(process.env.QUICK_GLOBAL_PER_HOUR, 5_000), concurrent: true,
});

export const meleeRateLimit = limiter({
  name: 'melee', max: positiveInt(process.env.MELEE_LIMIT_PER_HOUR, 20),
  windowMs: 60 * 60 * 1000, globalMax: positiveInt(process.env.MELEE_GLOBAL_PER_HOUR, 1_000), concurrent: true,
});

export const animalRateLimit = limiter({
  name: 'animal', max: positiveInt(process.env.ANIMAL_LIMIT_PER_HOUR, 20),
  windowMs: 60 * 60 * 1000, globalMax: positiveInt(process.env.ANIMAL_GLOBAL_PER_HOUR, 1_000), concurrent: true,
});
...
export const appAttestRateLimit = limiter({
  name: 'app-attest', max: positiveInt(process.env.APP_ATTEST_LIMIT_PER_HOUR, 120),
  windowMs: 60 * 60 * 1000,
  globalMax: positiveInt(process.env.APP_ATTEST_GLOBAL_PER_HOUR, 10_000),
  failClosed: true,
});

export const diagnosticRateLimit = limiter({
...
setInterval(() => {
  ...
  const pool = getDbPool();
  if (pool) void pool.query('DELETE FROM rate_limit_windows WHERE expires_at < NOW()').catch(() => undefined);
}, 10 * 60 * 1000).unref();
