...
function scrub(s: string, allowedPhrases: readonly string[]): string {
  let scrubbed = s.normalize('NFKD').replace(/\p{M}/gu, '').toLowerCase();
  for (const phrase of allowedPhrases) {
    if (phrase && scrubbed.includes(phrase)) scrubbed = scrubbed.split(phrase).join(' ');
  }
  return scrubbed;
}

function words(scrubbed: string): string[] {
...
function containsBlockedWord(s: string): boolean {
  ...
  if (nameWords.some(w => BLOCKED_WORDS.has(w))) return true;
  const joined = ` ${nameWords.join(' ')} `;
  if (BLOCKED_PHRASES.some(p => joined.includes(` ${p} `))) return true;
...
const BONE_BREAKING = new RegExp(
  '\\b(snap|snaps|snapping|crush|crushes|crushing|break|breaks|breaking|shatter|shatters|shattering|crack|cracks|cracking|splinter|splinters)\\b(?: \\w+){0,3} bones?\\b'
...
const BUILT_IN_NAMES = CREATURES.map(c => c.name.toLowerCase());

export type StoryPart = 'narration' | 'fact';
...
export function storyProblem(value: string, part: StoryPart, fighterNames: readonly string[] = []): string | null {
  if (/<\/?[a-z][^>]*>/i.test(value)) return 'markup';
  if (/https?:\/\/|www\./i.test(value)) return 'link';
  const allowed = [
  ...
  if (bones) return `phrase:${bones[0].trim()}`;
  if (part === 'narration') {
    const injury = tokens.find(w => INJURY_WORDS.has(w));
    if (injury) return `word:${injury}`;
 ...
 */
export function repairStory(value: string, part: StoryPart, fighterNames: readonly string[],
                            minSentences: number): string | null {
  if (storyProblem(value, part, fighterNames) === null) return value;
  ...
  if (kept.length === sentences.length || kept.length < minSentences) return null;
  return kept.join(' ');
}

/** Plain check for short AI text that isn't a battle story (e.g. a tier blurb). */
export function isSafeGeneratedText(value: string): boolean {
...
export function sanitizeFighterId(raw: unknown): SanitizeResult {
  if (typeof raw !== 'string') return { ok: false, value: '', error: 'must be a string' };
...
export function sanitizeEnvironment(raw: unknown): string | undefined {
  if (typeof raw !== 'string') return undefined;
  return ENVIRONMENTS.get(raw.trim().toLowerCase());
}

export function sanitizeTournamentContext(raw: unknown): string | undefined {
  if (typeof raw !== 'string') return undefined;
  const value = raw.toLowerCase();
  if (value.includes('championship') || value.includes('final')) return 'This is the tournament final.';
  if (value.includes('semifinal')) return 'This is a tournament semifinal.';
  if (value.includes('quarterfinal')) return 'This is a tournament quarterfinal.';
  if (value.includes('round of 16')) return 'This is a tournament round-of-16 battle.';
  if (value.includes('round of 32')) return 'This is a tournament round-of-32 battle.';
  if (value.includes('tournament')) return 'This is an early-round tournament battle.';
  return undefined;
}
