...
import { renderDashboardHtml } from '../views/adminDashboard';
...
import {
  adminRateLimit, adminReadRateLimit, battleRateLimit, meleeRateLimit, publicRateLimit, quickRateLimit,
} from '../middleware/rateLimit';
import {
  sanitizeEnvironment, sanitizeFighterId, sanitizeName, sanitizeTournamentContext,
} from '../middleware/sanitize';
import { getCustomCreatureReport, logCustomCreature, logCustomCreatureAttempt, purgeAllCustomCreatures } from '../services/customCreatureLogger';
...
import { displayName, isBuiltIn } from '../data/creatures';
import {
  logBattle,
  getAdminOverview,
...
function recordCustomMatchup(
  ...
  const label1 = fighter1Name ?? displayName(fighter1);
  const label2 = fighter2Name ?? displayName(fighter2);
  if (!isBuiltIn(fighter1) && fighter1Name) {
    logCustomCreature(fighter1Name, label2, environment, winnerId === fighter1);
  }
  if (!isBuiltIn(fighter2) && fighter2Name) {
    logCustomCreature(fighter2Name, label1, environment, winnerId === fighter2);
  }
}
...
function limitAdminRead(req: Request, res: Response, next: NextFunction): void {
  const limit = isAdminAuthorized(req) ? adminReadRateLimit : adminRateLimit;
...
router.post('/battle', requireAppAttest, battleRateLimit, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const generated = await idempotentOperation('battle', req.header('x-request-id'), async () => {
      if (fighter1Name) logCustomCreatureAttempt(fighter1Name);
      if (fighter2Name) logCustomCreatureAttempt(fighter2Name);
    ...
    res.once('finish', () => {
      if (generated.cacheHit) return; // Same request retry, not a new battle.
      recordCustomMatchup(
        fighter1, fighter2, fighter1Name, fighter2Name, result.winner, environmentName,
      );
      void logBattle({
        fighter1Id: fighter1,
        fighter2Id: fighter2,
  ...
  try {
    const generated = await idempotentOperation('melee', req.header('x-request-id'), async () => {
      for (const fighter of [...normTeamA, ...normTeamB]) {
        if (fighter.name) logCustomCreatureAttempt(fighter.name);
      }
    ...
    res.once('finish', () => {
      ...
      const label = (fighter: typeof normTeamA[number]) => fighter.name ?? displayName(fighter.id);
      for (const fighter of normTeamA) {
        if (!isBuiltIn(fighter.id) && fighter.name) {
          logCustomCreature(
            fighter.name,
            normTeamB.slice(0, 3).map(label).join(', '),
            environmentName,
      ...
      for (const fighter of normTeamB) {
        if (!isBuiltIn(fighter.id) && fighter.name) {
          logCustomCreature(
            fighter.name,
            normTeamA.slice(0, 3).map(label).join(', '),
            environmentName,
            result.winningTeam === 'B',
          );
        }
      }
...
router.get('/admin/custom-creatures', limitAdminRead, (req: Request, res: Response): void => {
...
router.get('/admin/assets/:font', (req: Request, res: Response): void => {
  if (!['Fredoka.ttf', 'Nunito.ttf'].includes(req.params.font)) {
    res.status(404).end();
    return;
  }
  res.setHeader('Cache-Control', 'public, max-age=86400');
  res.sendFile(resolve(__dirname, '../../public/admin', req.params.font));
});
...
router.get('/admin/dashboard', limitAdminRead, async (req: Request, res: Response): Promise<void> => {
  if (!isAdminAuthorized(req)) {
    res.redirect(303, '/api/admin/login');
    return;
  }
  ...
  try {
    const [overview, animals, recent] = await Promise.all([
      getAdminOverview(),
      getAnimalLeaderboard(500),
      getRecentActivity(200),
    ]);
    const custom = getCustomCreatureReport();
    const nonce = randomBytes(18).toString('base64');
    res.setHeader(
      'Content-Security-Policy',
      `default-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'; style-src 'unsafe-inline'; font-src 'self'; script-src 'nonce-${nonce}'`,
    );
