...
import { getCustomCreatureReport, logCustomCreature, purgeAllCustomCreatures } from '../services/customCreatureLogger';
...
router.get('/admin/dashboard', adminRateLimit, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.setHeader(
      ...
      `default-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'; style-src 'unsafe-inline'; script-src 'nonce-${nonce}'`,
    );
...
function renderDashboardHtml(data: {
  ...
  custom: ReturnType<typeof getCustomCreatureReport>;
  ...
  const ranking = (
    title: string,
    note: string,
    rows: typeof data.animals.topByWins,
    metric: (row: typeof rows[number]) => string,
