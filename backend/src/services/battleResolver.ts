...
export type Verdict =
  | {
      // High-confidence forced outcome — caller MUST set Claude's winner to this.
      kind: 'forced';
      winnerId: string;
      loserId: string;
      reason: string;
  ...
  | {
      // Close call or unknown fighter — caller lets Claude decide.
      kind: 'open';
      // For "open" matchups we still surface a soft prediction so the caller
      // can warn Claude / validate the response afterwards.
...
export interface ResolveArgs {
  fighter1Id: string;
  fighter2Id: string;
  /** Display name supplied by the client (used for custom creatures). */
  fighter1Name?: string;
  fighter2Name?: string;
 ...
 */
export function verdictPromptLine(v: Verdict, name1: string, name2: string,
                                  fighter1Id: string): string {
