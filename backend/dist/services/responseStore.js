...
exports.nextStoryVariant = nextStoryVariant;
...
async function nextStoryVariant(namespace, cacheInput, ttlMs, variants = STORY_VARIANTS) {
    if (variants <= 1)
        return 0;
    ...
    if (!pool) {
        ...
        if (rotationMemory.size >= MAX_MEMORY_ENTRIES && !entry) {
            const oldest = rotationMemory.keys().next().value;
            if (oldest)
                rotationMemory.delete(oldest);
        }
    ...
    try {
        ...
        const result = await pool.query(`INSERT INTO story_rotation (rotation_key, counter, expires_at)
       ...
       RETURNING counter`, [key, ttlMs]);
        return Number(result.rows[0]?.counter ?? 0) % variants;
    }
    catch (error) {
        console.error('[response-store] rotation failed:', error.message);
