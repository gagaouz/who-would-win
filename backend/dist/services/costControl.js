...
exports.initCostControl = initCostControl;
exports.runAiOperation = runAiOperation;
exports.isAiUnavailable = isAiUnavailable;
const database_1 = require("./database");
const MICRODOLLARS_PER_DOLLAR = 1000000;
...
function budgetMicrodollars(period) {
...
async function initCostControl() {
    if (initialized)
        return;
    if (initialization)
...
async function reserve(operation) {
    ...
    try {
        ...
        try {
            ...
            const current = await db.query(`SELECT period, microdollars FROM ai_spend_windows
          WHERE (period = 'day' AND period_key = $1)
             OR (period = 'month' AND period_key = $2)`, [dayKey, monthKey]);
            ...
            await db.query(`INSERT INTO ai_spend_windows (period, period_key, microdollars)
         ...
         ON CONFLICT (period, period_key) DO UPDATE
           SET microdollars = ai_spend_windows.microdollars + EXCLUDED.microdollars,
               updated_at = NOW()`, [dayKey, monthKey, amount]);
        ...
        if (error instanceof AiUnavailableError)
            throw error;
        console.error('[cost-control] reservation failed:', error.message);
...
function actualMicrodollars(message) {
    ...
    const usd = (message.usage.input_tokens * inputPerMillion
        + message.usage.output_tokens * outputPerMillion) / 1000000;
...
async function settle(reservation, actual) {
    ...
    if (!pool)
        return;
    try {
        await pool.query(`UPDATE ai_spend_windows
          SET microdollars = GREATEST(0, microdollars + $3), updated_at = NOW()
        WHERE (period = 'day' AND period_key = $1)
           OR (period = 'month' AND period_key = $2)`, [reservation.dayKey, reservation.monthKey, adjustment]);
    }
    catch (error) {
        // Settlement failures intentionally leave the conservative reservation.
        console.error('[cost-control] settlement failed:', error.message);
    }
}
async function runAiOperation(operation, call) {
