...
exports.StoryRejectedError = void 0;
exports.estimateCustomTier = estimateCustomTier;
exports.isCustomFighter = isCustomFighter;
exports.buildUserPrompt = buildUserPrompt;
exports.validateResult = validateResult;
exports.stripEmoji = stripEmoji;
exports.generateValidated = generateValidated;
exports.buildQuickUserPrompt = buildQuickUserPrompt;
...
const battleResolver_1 = require("./battleResolver");
...
function getPowerProfile(id, name) {
    const c = (0, creatures_1.getCreature)(id);
    if (c) {
...
const customTierCache = new Map();
const TIER_RUBRIC_SYSTEM = `You classify a creature into a POWER TIER from 1 to 10 for a kids' "who would win" game. ` +
...
async function estimateCustomTier(name, signal) {
    ...
    if (cached)
        return cached;
    try {
        const resp = await (0, anthropicClient_1.createMessage)('tier', {
            max_tokens: 80,
        ...
        const blurb = (0, sanitize_1.storyProblem)(rawBlurb, 'narration', [name]) === null ? rawBlurb : '';
        const result = { tier, blurb };
        if (customTierCache.size >= 1000) {
            const oldest = customTierCache.keys().next().value;
            if (oldest)
                customTierCache.delete(oldest);
        }
...
function isCustomFighter(id) {
    return !(0, creatures_1.isBuiltIn)(id);
}
/**
 ...
 */
async function estimateCustomTiers(fighter1Id, fighter1Name, fighter2Id, fighter2Name, signal) {
...
const SYSTEM_PROMPT = 'You are the storyteller for "Who Would Win?" — a fun, educational battle game for kids aged 6–12. ' +
...
function promptTier(id) {
    const c = (0, creatures_1.getCreature)(id);
    return c ? c.tier : null;
}
function buildUserPrompt(fighter1Id, fighter2Id, fighter1Name, fighter2Name, environmentName, tournamentContext, verdict) {
    const name1 = (0, creatures_1.displayName)(fighter1Id, fighter1Name);
    const name2 = (0, creatures_1.displayName)(fighter2Id, fighter2Name);
    ...
    const verdictLine = verdict ? (0, battleResolver_1.verdictPromptLine)(verdict, name1, name2, fighter1Id) : '';
    ...
    const tournamentLine = tournamentContext
        ...
        : '';
    const isDeity1 = (0, creatures_1.isDeity)(fighter1Id);
    const isDeity2 = (0, creatures_1.isDeity)(fighter2Id);
    ...
    const warn1 = environmentName ? (0, creatures_1.survivalWarning)(fighter1Id, name1, environmentName) : '';
    const warn2 = environmentName ? (0, creatures_1.survivalWarning)(fighter2Id, name2, environmentName) : '';
    ...
    if (tier1 !== null && tier2 !== null && tier1 !== tier2) {
        ...
        if (environmentName && (0, creatures_1.envModifier)(strongerId, environmentName) <= 0.10) {
    ...
    const profilesBlock = `FIGHTER PROFILES (use these as ground truth for size, weight, and ability — do NOT upgrade a small creature past its tier):\n` +
        `  • ${profile1}\n` +
        `  • ${profile2}\n\n` +
        tierGapLine;
    return (verdictLine +
...
class StoryRejectedError extends Error {
    constructor(reason) {
        super(`story rejected: ${reason}`);
        this.name = 'StoryRejectedError';
    }
}
exports.StoryRejectedError = StoryRejectedError;
function validateResult(data, fighter1Id, fighter2Id, fighterNames = [], minNarrationSentences = 2) {
    ...
    const narrationText = stripEmoji(narrationRaw).slice(0, 1200);
    if (narrationText.trim() === '')
        throw new StoryRejectedError('narration empty');
    const narration = (0, sanitize_1.repairStory)(narrationText, 'narration', fighterNames, minNarrationSentences);
    if (narration === null) {
        throw new StoryRejectedError(`narration ${(0, sanitize_1.storyProblem)(narrationText, 'narration', fighterNames)}`);
    }
    if (narration !== narrationText)
        logRepair('narration', narrationText, fighterNames);
    ...
    if (funFactText.trim() === '')
        throw new StoryRejectedError('funFact empty');
    const funFact = (0, sanitize_1.repairStory)(funFactText, 'fact', fighterNames, 1);
    if (funFact === null) {
        throw new StoryRejectedError(`funFact ${(0, sanitize_1.storyProblem)(funFactText, 'fact', fighterNames)}`);
    }
    if (funFact !== funFactText)
        logRepair('funFact', funFactText, fighterNames);
    ...
    const why = whyStripped !== '' && (0, sanitize_1.storyProblem)(whyStripped, 'narration', fighterNames) === null
...
function logRepair(field, original, fighterNames) {
    const part = field === 'narration' ? 'narration' : 'fact';
    console.warn(JSON.stringify({
        event: 'story_repaired', field, reason: (0, sanitize_1.storyProblem)(original, part, fighterNames),
    }));
}
 ...
 */
async function generateValidated(label, generate, parse) {
    let retryNote;
...
function buildArenaPrefill(fighter1Id, fighter2Id, name1, name2, environmentName) {
    const clean = (s) => s.replace(/^(SURVIVAL WARNING|NOTE): /, '').trim();
    const warn1 = clean((0, creatures_1.survivalWarning)(fighter1Id, name1, environmentName));
    const warn2 = clean((0, creatures_1.survivalWarning)(fighter2Id, name2, environmentName));
...
async function callClaude(fighter1Id, fighter2Id, topP, fighter1Name, fighter2Name, environmentName, tournamentContext, signal) {
    const name1 = (0, creatures_1.displayName)(fighter1Id, fighter1Name);
    const name2 = (0, creatures_1.displayName)(fighter2Id, fighter2Name);
    ...
    const { customTier1, customTier2 } = await estimateCustomTiers(fighter1Id, fighter1Name, fighter2Id, fighter2Name, signal);
    const verdict = (0, battleResolver_1.resolveBattle)({
    ...
    const result = await generateValidated('battle', async (retryNote) => {
        ...
        const response = await (0, anthropicClient_1.createMessage)('battle', {
            max_tokens: 450,
            top_p: topP,
    ...
    }, text => validateResult(JSON.parse(stripMarkdownFences(text)), fighter1Id, fighter2Id, [name1, name2]));
 ...
 */
function enforceVerdict(result, verdict, name1, name2, fighter1Id) {
    if (verdict.kind !== 'forced')
        return result;
    if (result.winner === verdict.winnerId)
        return result;
...
// the 1v1 Quick Fight.
function buildQuickUserPrompt(fighter1Id, fighter2Id, fighter1Name, fighter2Name, environmentName, verdict) {
    const name1 = (0, creatures_1.displayName)(fighter1Id, fighter1Name);
    const name2 = (0, creatures_1.displayName)(fighter2Id, fighter2Name);
...
const QUICK_SYSTEM_PROMPT = 'You are the storyteller for "Who Would Win?" — an educational battle game for kids aged 6–12. ' +
...
async function getQuickBattleResult(fighter1Id, fighter2Id, fighter1Name, fighter2Name, environmentName, signal) {
    ...
    let result = await generateValidated('quick', async (retryNote) => {
        const response = await (0, anthropicClient_1.createMessage)('quick', {
            max_tokens: 360,
            top_p: 0.85,
        ...
        return block.text;
    }, text => validateResult(JSON.parse(stripMarkdownFences(text)), fighter1Id, fighter2Id, [name1, name2], 1));
...
async function getBattleResult(fighter1Id, fighter2Id, fighter1Name, fighter2Name, environmentName, tournamentContext, signal) {
    ...
    return callClaude(fighter1Id, fighter2Id, 0.85, fighter1Name, fighter2Name, environmentName, tournamentContext, signal);
}
