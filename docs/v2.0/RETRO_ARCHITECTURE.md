...
## Verified 1.1.7 source findings
...
| Custom art | `AnimalImageService` resolves Wikipedia imagery, then a Pollinations URL. Its UIImage and URL caches are bounded in-memory dictionaries. `/api/animal` returns emoji/category/color, **not sprites**. | Replace the artwork path with the approved on-device deterministic avatar kit. No hosted generator or custom-art persistence service is required; the prototype's authored Mossback is only an art reference. |
...
| `RetroCreatureArtwork` | One SwiftUI entry point for catalog/custom portraits, icons, stickers and share-card imagery. Uses the idle frame and the same creature appearance as battle. | Network work or gameplay mutation during rendering. |
| `RetroAssetStore` / local avatar composer | Bundled manifest lookup, validated texture loading, deterministic custom recipe composition, bounded in-memory caching and asset preparation. Custom art has no API or persistent disk dependency. | Winners, HP, coin charging, image-provider requests. |
...
**Owner decision, September 26, 2026: custom retro artwork is composed entirely on the device from a curated sprite kit.** Recurring paid image generation was declined. The earlier proposal for provider credentials, backend generation jobs, immutable artifact URLs, hosted retention, and persisted custom sprite downloads is superseded and is not a release gate.

1. **Preserve custom play.** Retain existing validated names, content filtering, first-free/coin/rewarded-ad selection behavior, random participant IDs, accepted battle rules, Next Challenger, embedded tournament records, and sharing. Art preparation never spends coins or makes custom play depend on a service.
2. **Choose a local recipe.** Match recognizable animal/type words to curated bundled bases and approved palettes/accessories. Unknown names select a stable fantasy avatar. Recipes describe appearance and anatomy only; they do not influence combat stats, winners or terminal health.
3. **Keep identity deterministic.** Normalize the display name consistently and use an explicit stable recipe/version policy, not Swift's randomized process hash or the participant UUID. The same retained name recreates the same appearance after relaunch and in different views. Preserve random gameplay IDs and existing Codable records; old photo URLs may remain decodable but are not fetched for retro artwork.
4. **Compose within a bounded budget.** Render pixel artwork from bundled assets and cache derived images in memory. No API, provider account, image-server response, persistent image file, or disk-cache index is needed. Clear/evict memory freely and recreate the image on demand. Validate canvas bounds, transparency, anchors, palette and recipe asset references at build/test time.
5. **Share one artwork contract.** Picker selection, battle, results, bracket portraits, and exports call the same asset provider. Prepare a custom image before synchronous `ImageRenderer` export. Pin the appearance in an active battle session so a view refresh cannot change it during an action. First-time offline creation uses exactly the same local path.
6. **Describe the output honestly.** The typing preview remains an intentional pencil token. On commitment it becomes a finished retro avatar; it is not waiting for a later paid/network generation. User-facing wording may say “We'll give your fighter a retro avatar, made on your device.” Exact semantic likeness, unique appearance for every possible name, and arbitrary offline text-to-image generation are not promised. A missing/corrupt bundled asset uses a local fallback and cannot block a fight.

Acceptance includes recognizable quadruped, bird, sea, serpent, arthropod and humanoid/fantasy names; unknown names; equivalent names with different UUIDs; two-custom and custom-versus-catalog battles; punctuation, length limits and blocked input; Next Challenger; all share renderers; cache eviction; relaunch; an upgraded in-progress bracket with custom entrants; and first-use offline artwork. Assert that the art path makes no image-service calls and cannot cause duplicate custom unlock spending. Review visual variation without requiring a unique or literal likeness for every arbitrary input.

Keep appearance keys separate from gameplay identity. Raw names need not become files, URLs, analytics keys or server cache identifiers because artwork remains local and derivable. Existing battle requests and their privacy/validation rules remain separate from this artwork change; local artwork does not imply all custom battle processing is offline. See [CUSTOM_ART_SERVICE.md](CUSTOM_ART_SERVICE.md) for implementation status and [AnimalPickerViewModel](../../ios/WhoWouldWin/ViewModels/AnimalPickerViewModel.swift) for the retained random-ID contract.
...
- **Storage:** bundle catalog/arena essentials and the full custom avatar kit. Derive custom images from the retained name and versioned local recipe policy. In-memory images may be evicted and recreated; no downloaded file, disk index, or persistent custom image is required. Retain existing embedded custom `Animal` records and random IDs in tournament saves.
- **Validation:** validate bundled atlas/frame bounds, alpha, anchors and every recipe asset reference. Bound composed image dimensions and decoded memory; reuse equivalent recipes. Missing kit data must resolve to a bundled/local fallback and must not block an accepted result. There is no custom-art download/retry state in the release contract.
...
| 4. Custom production path | Recognized animal/type words choose appropriate local recipes; unknown names receive stable fantasy avatars. Equivalent normalized names remain consistent across random participant IDs, views, sharing, relaunch and saved brackets. First-use offline artwork works without API or disk cache. Verify accurate avatar wording, no image-service calls, no duplicate spend and safe missing-asset behavior; exact semantic likeness is not required. |
