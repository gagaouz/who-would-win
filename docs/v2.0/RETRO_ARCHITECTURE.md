...
Keep appearance keys separate from gameplay identity. Raw names need not become files, URLs, analytics keys or server cache identifiers because artwork remains local and derivable. Existing battle requests and their privacy/validation rules remain separate from this artwork change; local artwork does not imply all custom battle processing is offline. See [CUSTOM_AVATARS.md](CUSTOM_AVATARS.md) for implementation status and [AnimalPickerViewModel](../../ios/WhoWouldWin/ViewModels/AnimalPickerViewModel.swift) for the retained random-ID contract.
